...
const JSList = {
	...
	},
	{
		"name": "BalooProxy",
		"navigations": 1,
		"locate": "<h1>Enter the text you see in the picture</h1>"
	}]
}
		...
		playwright.firefox.launch({
			...
			if (JS) {
				...
				if (JS.name == "VShield") {
					...
					await page.mouse.up();
				} else if (JS.name == "BalooProxy") {
					log('(' + 'JSDetect'.green + `)` + ' BalooProxy Detected. '.magenta + `(${JS.name})`.yellow);
					log('(' + 'JSDetect'.green + `)` + ' BalooProxy Detected. '.magenta + `(${JS.name})`.yellow);
					log('(' + 'JSDetect'.green + `)` + ' BalooProxy Detected. '.magenta + `(${JS.name})`.yellow);
					log('(' + 'JSDetect'.green + `)` + ' BalooProxy Detected. '.magenta + `(${JS.name})`.yellow);
					log('(' + 'JSDetect'.green + `)` + ' BalooProxy Detected. '.magenta + `(${JS.name})`.yellow);
					browser.close();
					reject('BalooProxy Detected');
					spawn('pkill', ['playwright']);
				}
